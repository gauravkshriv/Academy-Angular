...
PortalData(){
  this.activatedRoute.queryParams.subscribe( (params: Params) => {
	...
	localStorage.setItem("Userdetails", this.decryptedData[0].Userdetails);
	 ...
	 console.log(">>>>>>>>>>>>BasicStatus", this.decryptedData[0].Userdetails._upst)

	//  if (this.decryptedData[1].username == null) {
	// 	localStorage.setItem("kycstatus", "true");
	// 	console.log(">>>>>>>>>>>>kycststus", this.decryptedData[1].username)
	//   }

	//  else if( this.decryptedData[0].Userdetails._upst === "COMPLETE")  {
	...
	//  localStorage.setItem("fullname", this.decryptedData[5].fullname);
		   swal({
			title: "Thank You!!!",
			text: "Welcome to RME Academy Portal.",
			icon: "success",
			// button: false,
